...
stats1 = dbc.Card(
    [
        html.Div(html.H4(f'Total Patient Records: {num_records}', style={'fontSize': 20})),
...
stats2 = dbc.Card(
    [
        html.Div(html.H4(f'Total Number of Hospitals: {num_hospitals}', style={'fontSize': 20})),
        html.Div(html.H4(f'Total Number of num_doctors: {num_doctors}', style={'fontSize': 20}))
    ],
    body=True
)
stats3 = dbc.Card(
    [
        html.Div(html.H4(f'Total Number of Insurance Provider: {num_ins_provider}', style={'fontSize': 20})),
        html.Div(html.H4(f'Total Number of Medical Conditions: {num_medical_conditions}', style={'fontSize': 20}))
    ],
    body=True,
)
...
age_distribution = dbc.Card(
    [   
        ...
        html.Div(dcc.Graph(id="age-distribution", responsive=True, style={'width': '100%', 'height': '100%'}))
    ],
    body=True
)
...
medical_condition_distribution = dbc.Card(
    [   
        ...
        html.Div(dcc.Graph(id="condition-distribution", responsive=True, style={'width': '100%', 'height': '100%'}))
    ],
    body=True
)
...
insurance_provider_distribution = dbc.Card(
    [   
        ...
        html.Div(dcc.Graph(id="insurance-comparison", responsive=True, style={'width': '100%', 'height': '100%'}))
    ],
    body=True
)
...
billing_amount_distribution = dbc.Card(
    [   
        ...
        html.Div(dcc.Slider(
            ...
            marks={int(value): f'{int(value):,}' for value in df['Billing Amount'].quantile([0, 0.25, 0.50, 0.75, 1.0]).values},
            step=100
        )),
        html.Div(dcc.Graph(id="billing-distribution", responsive=True, style={'width': '100%', 'height': '100%'}))
    ],
    body=True
)
...
trends_in_admission_distribution = dbc.Card(
    [   html.Div(html.H5('Trends in Admission')),
        html.Div(dcc.Graph(id="admission-trend-distribution", responsive=True, style={'width': '100%', 'height': '100%'}))
    ],
    body=True
)
...
def medical_condition_distribution(gender):
    if gender:
        filtered_df = df[df['Gender']==gender]
    else:
        filtered_df = df
...
def insurance_provider_distribution(gender):
    if gender:
        filtered_df = df[df['Gender']==gender]
    else:
        filtered_df = df
...
def billing_amount_distribution(gender, amount):
    if gender:
        filtered_df = df[df['Gender']==gender]
    else:
        filtered_df = df
...
def trends_in_admission_distribution(gender, condition, chart_type):
    if gender:
        filtered_df = df[df['Gender']==gender]
    else:
        filtered_df = df
